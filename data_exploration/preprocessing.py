...
def resize_images(x_train, width, height):
	(n,w,h) = x_train.shape
	w = int(width)
	h = int(height)
...
# image_array_resized = resize_images(image_array, 512, 512)
# np.savez(df_preprocess_base_folder+"train_512.npz", image_array=image_array_resized, 
# 		filename_list=filename_list)
# image_array_resized = resize_images(image_array, 256, 256)
# np.savez(df_preprocess_base_folder+"train_256.npz", image_array=image_array_resized, 
# 		filename_list=filename_list)

# image_array_resized = resize_images(image_array, 224, 224)
# np.savez(df_preprocess_base_folder+"train_224.npz", image_array=image_array_resized, 
# 		filename_list=filename_list)

image_array_resized = resize_images(image_array, 128, 128)
np.savez(df_preprocess_base_folder+"train_128.npz", image_array=image_array_resized, 
		filename_list=filename_list)
...
# 		filename_list=filename_list)
# image_array_resized = resize_images(image_array, 224, 224)
# np.savez(df_preprocess_base_folder+"test_224.npz", image_array=image_array_resized, 
# 		filename_list=filename_list)
